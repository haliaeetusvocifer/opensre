...
description: 'How OpenSRE structures execution data'
---

OpenSRE organizes execution data into a small set of core entities that reflect how real workloads run: runs, tasks, tools, containers, and hosts.

This data model allows OpenSRE to map low-level execution signals to the way teams reason about pipelines and infrastructure, without relying on workflow metadata, logs, or application instrumentation.
...
- OpenSRE/collect observes execution events at the operating system level
- These events are correlated into structured entities
- Higher-level products (OpenSRE/tune and OpenSRE/sweep) operate on this shared model
...
OpenSRE identifies tools based on observed process execution, not logs or configuration. Even tools that produce no logs are captured as first-class entities.
...
OpenSRE does not require containers to be present, but when they are used, container context is preserved and reflected in the data model.
...
This structure allows OpenSRE to:
...
OpenSRE correlates execution events using identifiers exposed by the operating system, including:
...
This data model is the foundation for OpenSRE's higher-level capabilities.
...
OpenSRE/tune and OpenSRE/sweep operate on this shared structure rather than raw telemetry.
...
OpenSRE models how workloads execute, not what they compute.

## Orchestrator terminology mapping (reference)

OpenSRE's data model is framework- and language-agnostic. The table below shows how OpenSRE entities typically align with common orchestrator concepts. Exact mappings may vary by workflow engine and configuration.
...
| OpenSRE concept | Common equivalents |
| --- | --- |
...
<Tip>This mapping is provided for orientation only. OpenSRE does not depend on orchestrator metadata to build its execution model.</Tip>

## When to read this page

This page is most useful if you:

- Want to understand how OpenSRE structures execution data
- Are integrating OpenSRE data into external systems
- Need clarity on attribution boundaries and terminology
- Are evaluating OpenSRE for complex or regulated environments
