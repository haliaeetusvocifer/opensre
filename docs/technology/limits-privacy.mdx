...
description: 'What OpenSRE does and does not collect'
---

OpenSRE is designed to provide execution insight while minimizing data exposure. It observes how workloads run, not what they compute or the data they process.

This page explains OpenSRE's intentional limits, privacy boundaries, and data handling principles.

## What OpenSRE collects

OpenSRE collects execution metadata derived from operating system–level signals.
...
## What OpenSRE does not collect

OpenSRE explicitly does not collect or inspect:
...
<AccordionGroup>
  <Accordion title="Application and scientific data" icon="database">
    ...
    <Note>OpenSRE may observe that a file was accessed, but never reads or captures file contents. This behavior can be verified in the open-source OpenSRE/collect implementation.</Note>
  </Accordion>
  <Accordion title="Source code and runtime internals" icon="code">
    ...
    <Note>OpenSRE operates at the process and kernel level, not inside language runtimes.</Note>
  </Accordion>
  <Accordion title="Secrets and sensitive configuration" icon="key">
    ...
    <Note>OpenSRE does not inspect process memory or application configuration.</Note>
  </Accordion>
  <Accordion title="Application- or domain-level semantics" icon="microscope">
    ...
    <Note>While OpenSRE can observe which binaries or commands were executed, it does not infer what those commands mean within an application or domain.</Note>
  </Accordion>
</AccordionGroup>
...
OpenSRE follows a data-minimization approach:
...
OpenSRE maintains a small set of internal allowlists and denylists to focus collection on meaningful execution activity and reduce unnecessary data.
...
Maintaining explicit allowlists and denylists helps OpenSRE:
...
- Only derived metadata is transmitted to the OpenSRE backend
- Payload data is never exported
- Data retention and access are governed by account-level configuration

OpenSRE separates collection, correlation, and analysis to reduce exposure.
...
OpenSRE observes execution within the boundaries enforced by the operating system, container runtime, and cloud provider.

## Transparency and open source

The core OpenSRE agent (OpenSRE/collect) is open source. The repository documents how execution signals are collected, filtered, and structured, and makes it possible to independently review what data is gathered and what is explicitly excluded.

This transparency supports security reviews and helps teams verify OpenSRE's data-collection boundaries.

<Card title="OpenSRE/collect on GitHub" icon="github" href="https://github.com/Tracer-Cloud/tracer-client">
...
- Evaluate OpenSRE's suitability for production workloads
...
OpenSRE provides execution visibility without inspecting application data. By limiting collection to system-level execution metadata, applying conservative filtering, and enforcing clear boundaries, OpenSRE delivers performance and cost insight while preserving privacy and security.
