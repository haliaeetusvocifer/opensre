---
title: 'How to use OpenSRE with Grafana'
...
OpenSRE complements Grafana by observing how tasks, tools, and processes actually execute at runtime and organizing this behavior by pipeline, run, and execution unit.
...
A dashboard shows high instance utilization during a pipeline run. OpenSRE reveals that:
...
<iframe
  ...
  title="Grafana vs Grafana+OpenSRE capabilities comparison"
/>
...
OpenSRE is not a general-purpose visualization platform.
...
## When to use OpenSRE with Grafana

OpenSRE is most useful alongside Grafana when teams need to:
...
OpenSRE focuses on execution behavior. Grafana continues to provide visualization and alerting for reported telemetry.

## Summary

Grafana visualizes metrics, logs, and traces collected from many systems. OpenSRE adds execution-level visibility that shows how pipelines, tasks, and tools actually behave at runtime and how resource usage and cost map to real work. Together, they provide complementary perspectives without overlap.
