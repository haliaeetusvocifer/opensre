---
title: 'How to use OpenSRE with Prometheus'
...
OpenSRE complements Prometheus by observing how tasks, tools, and processes actually execute at runtime and organizing this behavior by pipeline, run, and execution unit.
...
Prometheus shows sustained CPU utilization during a pipeline run. OpenSRE reveals that:
...
<iframe
  ...
  title="Prometheus vs OpenSRE+Prometheus capabilities comparison"
/>
...
OpenSRE is not a general-purpose metrics backend.
...
OpenSRE stores and analyzes execution-derived metrics it observes, scoped to pipeline workloads.

## When to use OpenSRE with Prometheus

OpenSRE is most useful alongside Prometheus when teams need to:
...
OpenSRE focuses on execution behavior. Prometheus continues to provide metric collection and querying across broader systems.

## Summary

Prometheus records metrics that systems and applications report. OpenSRE adds execution-level visibility that shows how pipelines, tasks, and tools actually behave at runtime and how resource usage and cost map to real work. Together, they provide complementary perspectives without overlap.
