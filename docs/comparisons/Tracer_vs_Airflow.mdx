---
title: 'How to use OpenSRE with Apache Airflow'
...
OpenSRE complements Airflow by exposing execution behavior: CPU, memory, disk, and network usage, during task execution, without changing DAG definitions or operator configuration.
...
<iframe
  ...
  title="Airflow + OpenSRE Pipeline Intelligence"
/>

## Example: diagnosing a slow Airflow task

An Airflow task consistently exceeds its expected runtime. Logs show normal progress. OpenSRE reveals:
...
OpenSRE makes this distinction explicit by observing runtime behavior instead of inferring it from task duration alone.
...
<iframe
  ...
  height="700px"
  style={{ border: 'none', borderRadius: '0.125rem', background: 'transparent', overflow: 'hidden' }}
  title="Airflow vs OpenSRE+Airflow Radar Chart"
/>
...
OpenSRE is not an orchestration system.
...
Airflow remains responsible for orchestration. OpenSRE makes execution behavior visible.

## When to use OpenSRE with Airflow
...
OpenSRE operates independently of Airflow and supports tasks written in any language or toolchain.

## Summary

Apache Airflow defines and schedules workflows. OpenSRE adds execution-level visibility that shows how tasks actually behave at runtime. Together, they provide both control and insight, without changes to existing DAGs.
