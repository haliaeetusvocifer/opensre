---
title: 'How to use OpenSRE with Flyte'
sidebarTitle: 'Flyte'
...
OpenSRE complements Flyte by exposing execution behavior: CPU, memory, disk, and network usage, during task execution, without modifying Flyte task definitions, container images, or execution semantics.

<Note>
For a conceptual overview, see [How OpenSRE fits in your stack](/comparisons/overview).
</Note>
...
OpenSRE observes execution directly from the host and container runtime and adds:
...
<iframe
  ...
  title="Flyte + OpenSRE Pipeline Intelligence"
/>
...
This indicates an I/O-bound workload rather than insufficient compute. Increasing CPU requests would not reduce runtime. OpenSRE makes this visible by observing execution behavior directly, rather than inferring causes from task duration.
...
<iframe
  ...
  height="700px"
  style={{ border: 'none', borderRadius: '0.125rem', background: 'transparent', overflow: 'hidden' }}
  title="Flyte vs OpenSRE+Flyte Radar Chart"
/>

This comparison highlights the difference between Flyte's task-level orchestration visibility and OpenSRE's execution-level observation.

## What OpenSRE does not replace

OpenSRE is not an orchestration framework.
...
Flyte remains responsible for orchestration and correctness. OpenSRE makes execution behavior visible.

## When to use OpenSRE with Flyte

OpenSRE is most useful when teams need to:
...
OpenSRE operates independently of Flyte and supports workflows written in any language or toolchain.

## Summary

Flyte defines and orchestrates tasks and workflows with strong guarantees around correctness and reproducibility. OpenSRE adds execution-level visibility that shows how those tasks actually behave at runtime. Together, they provide both control and insight, without changes to existing Flyte workflows.
