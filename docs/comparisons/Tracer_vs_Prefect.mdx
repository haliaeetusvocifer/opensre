---
title: "How to use OpenSRE with Prefect"
...
OpenSRE complements Prefect by exposing execution behavior: CPU, memory, disk, and network usage, during flow runs, without modifying flow definitions or task code.
...
<iframe
  ...
  title="Prefect + OpenSRE Pipeline Intelligence"
/>

## Example: diagnosing slow Prefect tasks

A Prefect task reports a long runtime but no errors. OpenSRE shows:
...
OpenSRE makes this distinction explicit by observing runtime behavior rather than inferring it from task duration alone.
...
<iframe
  ...
  height="700px"
  style={{ border: 'none', borderRadius: '0.125rem', background: 'transparent', overflow: 'hidden' }}
  title="Prefect vs OpenSRE+Prefect Radar Chart"
/>
...
OpenSRE is not a workflow orchestrator.
...
Prefect remains responsible for orchestration. OpenSRE makes runtime behavior visible.

## When to use OpenSRE with Prefect
...
OpenSRE operates independently of Prefect and supports workflows written in any language or toolchain.

## Summary

Prefect defines and orchestrates workflows through flows and tasks. OpenSRE adds execution-level visibility that shows how those tasks actually behave at runtime. Together, they provide both control and insight, without changes to existing workflows.
