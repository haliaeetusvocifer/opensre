---
title: "How to use OpenSRE with Seqera"
sidebarTitle: "Seqera"
...
OpenSRE complements Seqera by exposing execution behavior: CPU, memory, disk, and network usage, and more during pipeline runs. It collects this telemetry without modifying workflows or task definitions.
...
<iframe
  ...
  title="OpenSRE/collect execution panel"
/>

## Example: identifying I/O-bound tasks

A pipeline task requests many CPUs and a large memory allocation. During execution, OpenSRE shows:
...
OpenSRE makes this distinction explicit by observing runtime behavior rather than inferring it from configuration.
...
OpenSRE is not a workflow engine.
...
Seqera remains responsible for orchestration. OpenSRE makes runtime behavior visible.

## When to use OpenSRE with Seqera
...
OpenSRE operates independently of the workflow engine and supports pipelines across multiple frameworks and languages.

## Summary

Seqera and Nextflow define and orchestrate scientific workflows. OpenSRE adds execution-level visibility that shows how resources are actually used at runtime. Together, they support reliable execution and informed optimization decisions, without changes to existing workflows.
